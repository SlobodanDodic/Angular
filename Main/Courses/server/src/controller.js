...
module.exports.queryApplications = queryApplications;
module.exports.saveApplication = saveApplication;

const coursesFileName = "courses";
const applicationsFileName = "applications";

function queryApplications(req, res) {
    model.load(applicationsFileName, function(applications) {
        let result = [];
        if (req.params.eMail) {
            result = applications.filter(function(application) {
                if (application.eMail) {
                    return application.eMail == req.params.eMail;
                }
                return false;
            });
        }
...
function saveApplication(req, res) {
    model.load(coursesFileName, function (courses) {
        model.load(applicationsFileName, function(applications) {
            let application = req.body;

            if (application.course?.id <= 0 || application.eMail == "") {
                res.status(400).json({});
                return;
            }

            let existingApplication = undefined;
            for (let itApplication of applications) {
                if (itApplication.course.id == application.course.id && itApplication.eMail == application.eMail) {
                    existingApplication = itApplication;
                    break;
                }
            }
            ...
            for (let itCourse of courses) {
                if (itCourse.id == application.course.id) {
                    course = itCourse;
                    break;
                }
            }
            ...
            if (applications.length > 0) {
                lastId = parseInt(applications[applications.length - 1].id);
            }
            application.id = lastId + 1;
            application.course = course;
            application.date = new Date();

            applications.push(application);
            model.save(applicationsFileName, applications);
...
function query(req, res) {
    model.load(req.params.entity, function(entities) {
		...
		if(req.query.filter) {
            var weeksTo = parseInt(req.query.filter.weeksTo, 10);
			if(weeksTo !== undefined && !isNaN(weeksTo)) {
				entities = entities.filter(function(obj) {
					return parseInt(obj.weeks, 10) <= weeksTo;
				});
			}
		}
...
function save(req, res) {
    ...
    model.load(req.params.entity, function(entities) {
        ...
        if(entities.length > 0){
            lastId = parseInt(entities[entities.length - 1].id);
        }
...
function showRelationship(req, res) {
    model.load(req.params.related, function(entities) {
        for(var i = 0, n = entities.length; i < n; i++) {
            var elem = entities[i];
            if(req.params.relatedId === elem.id.toString() && req.params.entityId == elem[req.params.entity]) {
                res.status(200).json(elem);
                return;
            }
        }
